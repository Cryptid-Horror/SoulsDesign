        ...
        if (isset($data['attachment_id'])) {
            foreach ($data['attachment_id'] as $key => $attachment_id) {
                ...
                switch ($data['attachment_type'][$key]) {
                    case 'Item':        $attach = \App\Models\Item\Item::find((int) $attachment_id);
                        break;
                    ...
                    case 'Figure':      $attach = \App\Models\WorldExpansion\Figure::find((int) $attachment_id);
                        break;
                    case 'Fauna':       $attach = \App\Models\WorldExpansion\Fauna::find((int) $attachment_id);
                        break;
                    case 'Flora':       $attach = \App\Models\WorldExpansion\Flora::find((int) $attachment_id);
                        break;
                    ...
                    case 'Event':       $attach = \App\Models\WorldExpansion\Event::find((int) $attachment_id);
                        break;
                    case 'Location':    $attach = \App\Models\WorldExpansion\Location::find((int) $attachment_id);
                        break;
