...
@php
        ...
        $health =$stats->shift();
    @endphp
    <b>Character Health</b>
    <div class="progress bg-dark rounded-0 mt-2 clip-br" style="height: 2em;">
        <div class="progress-bar bg-danger text-dark progress-bar-striped progress-bar-animated active h3" role="progressbar" style="width: 10%" aria-valuenow="{{ $health->current_count}}" aria-valuemin="0" aria-valuemax="{{ $health->current_count }}" style="height:100%; width:{{ isset($health->current_count) ? $health->current_count : 100 }}%">
